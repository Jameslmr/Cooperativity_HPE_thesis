...
def Ph(pH,pka1,M,f,r):
    return mp.power((1+(mp.power(10,(pka1-pH)))),(f*M))
...
def Pa(pH,Gh,pka2,pka3,M,f2,r):
    return mp.power((mp.exp(-Gh*(r-1))),M)*mp.power((1+(mp.power(10,(pka2-pH)))),(f2*M))*mp.power(((1+(mp.power(10,(pka3-pH))))),((1-f2)*M))
...
def theta2(pH,pka1,Gh,pka2,pka3,M,f,f2,r):
     return np.array(((mp.power(10,(-pH))/(M*Pt(pH,pka1,Gh,pka2,pka3,M,f,f2,r)))*(((f*M)*mp.power(10,(pka1))*mp.power((1+mp.power(10,(pka1-pH))),((f*M)-1)))+(mp.power(mp.exp(-Gh*(r-1)),M)*((mp.power((1+mp.power(10,(pka3-pH))),((1-f2)*M))*((f2*M)*mp.power(10,(pka2))*mp.power((1+mp.power(10,(pka2-pH))),((f2*M)-1))))+(mp.power((1+mp.power(10,(pka2-pH))),((f2)*M))*(((1-f2)*M)*mp.power(10,(pka3))*mp.power((1+mp.power(10,(pka3-pH))),(((1-f2)*M)-1)))))))),dtype=float)
...
def Pa_n(pH,Gh2,pka2,M,r):
    return mp.power((mp.exp(-Gh2*(r-1))*(1+(mp.power(10,(pka2-pH))))),M)
...
params=fit_line(pH_range, init_pka1, init_Gh,init_pka2,init_pka3,init_M,init_f,init_f2,init_r)
line4, = plt.plot(pH_range,fh_fit(pH_range,params[0],params[1]), lw=2, label="$M_{eff}=$"+str(np.round(params[1],decimals=1)))
...
def update(val):
    ...
    line4.set_label('$M_{eff}=$'+str(np.round(params[1],decimals=1)))
    legend.remove()
    ax.legend()
